...
- 主要变更：
  - 在药品种子 SQL 中新增 10 条演示用药品记录，覆盖抗生素、心血管、消化系统、抗过敏、维生素等常见分类
  - 保持 `ON DUPLICATE KEY UPDATE` 逻辑不变，便于重复初始化时幂等更新
- 备注：
  - 新增数据会在重新执行数据库初始化后生效；如当前容器已启动，可通过 `docker compose down -v && docker compose up -d --build` 重新灌库
