...
一个基于 J2EE 技术栈落地的药物管理系统项目，当前已完成前后端工程骨架、后端核心业务主链路、最小认证基线与阶段性测试验证；现阶段继续按执行清单推进认证收口、文档同步和容器化演示准备。
...
- 当前已完成最小认证接入：
  - 后端 `/api/**` 已支持通过 `Authorization: Bearer <token>` 解析当前会话
  - 后端仍兼容既有 `X-User-Id`、`X-User-Name`、`X-User-Role` 头，便于阶段性联调
  - 后端已提供 `POST /api/auth/login` 用于演示账号登录
  - 后端已提供 `POST /api/auth/register` 用于演示账号注册
  - 后端已提供 `GET /api/auth/me` 用于返回当前登录用户
  - 认证账号已落到 `user_account` 表，默认演示账号已进入数据库初始化脚本
  - 前端登录页已改为调用后端登录接口，不再在浏览器端保存明文密码
  - 前端注册页已改为调用后端注册接口，不再在浏览器端维护账号密码列表
  - 用户号首位仅用于登录页角色提示，不再作为前端权限判定依据：
    - `1` 开头为药师
    - `2` 开头为医生
  - 默认演示账号：
    - `1001 / pharm123`
...
- 当前已完成库存模块联调页面：
  - 已支持库存分页查询与库存流水查询
  - 已支持入库、出库、盘点三类操作
  - 当前库存请求中的 `operatorName` 已由前端从当前登录用户自动注入
  - 后端库存服务当前会优先从认证上下文解析操作者姓名，旧字段仅兼容回退
  - 已修复出库 / 盘点成功提示文案错误
- 当前已完成预警模块联调页面：
  - 已支持低库存预警查询
  - 已支持临期 / 过期预警查询
...
- 当前已完成处方模块联调页面：
  ...
  - 已支持医生直接建方与药师代开发起
  - 已支持医生授权/拒绝代开
  - 已支持医生提交草稿、药师审核、药师发药、取消处方
  - 已通过当前登录用户自动注入 `createdBy*`、`operator*` 相关请求字段
  - 后端处方服务当前已优先使用认证上下文中的 `createdBy*`、`operator*` 信息
- 当前已完成面向演示的容器化收口：
  - 前端镜像已切换为生产构建产物，不再依赖 `vite dev server`
  - 前端在容器构建时改为通过 Nginx 统一转发现有 `/api/**` 请求，避免页面内写死容器地址
  - `docker-compose.yml` 已按“mysql + backend + frontend + nginx”统一编排
  - 已完成前端生产构建、后端打包、Compose 结构与 Nginx 配置语法校验

当前前端路由结构：

- 主路由 `/`：系统主入口，展示模块按钮
- 分路由 `/drugs`：药品管理
- 分路由 `/inventories`：库存管理
- 分路由 `/inventories/records`：库存流水独立页
- 分路由 `/warnings`：预警查询
- 分路由 `/prescriptions`：处方工作台
- 分路由 `/prescriptions/new`：医生新建处方独立页
- 分路由 `/prescriptions/:id`：处方详情独立页
...
- 待完成部分：具备 Docker 命令的机器上完成 `docker compose up` 实跑验证、部署回归验证、剩余认证字段继续向后端上下文内收

### 3.2 后续推进原则

后续实现建议遵循以下原则：

1. 先完成主链路验证，再进入 Docker 与部署验证
